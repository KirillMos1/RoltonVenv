    ...
    if cmd:
        ...
        if cmd[0] == "help":
            if len(cmd) == 1:
                ...
                logger.write(
                    f"[{datetime.datetime.now()}] Succesful execute '{cmd[0]}'\n"
                )
                logger.flush()
                ...
                if cmd[1] == "errors":
                    ...
                    print(
                        "Класс ошибок 0x0000001x - ошибки в системе пользователей / фатальные ошибки"
                    )
                    print(" - 0x00000011 - пользователь уже существует")
                    print(" - 0x00000012 - пользователь не существует")
                    print(
                        " - 0x00000013 - файловая система повреждена (также является типом ошибки для всех встроеенных ошибок)"
                    )
                    ...
                    print(
                        " - 0x00000043 - модуль поврежден (также является типом ошибки для всех встроеенных ошибок)"
                    )
                    ...
                    print(
                        f"COMMAND_ARGUMENT_ERROR (0x00000022): неизвестный аргумент функции '{cmd[1]}'"
                    )
                    logger.write(
                        f"[{datetime.datetime.now()}] Failed execute '{cmd[0]}' code 0x00000022\n"
                    )
                    logger.flush()
        elif cmd[0] == "exit":
            if len(cmd) == 1:
            ...
            try:
                ...
                logger.write(
                    f"[{datetime.datetime.now()}] Failed execute '{cmd[0]}' BUILTIN_ERROR '{e}'\n"
                )
                logger.flush()
            ...
            if len(cmd) > 2:
                if cmd[1] == "calc":
                    ...
                    logger.write(
                        f"[{datetime.datetime.now()}] Succesful execute utilite '{cmd[1]}'\n"
                    )
                    logger.flush()
                    ...
                    logger.write(
                        f"[{datetime.datetime.now()}] Failed execute '{cmd[0]}' code 0x00000031\n"
                    )
                    logger.flush()

            else:
                print(
                    "COMMAND_NEED_ARGUMENT_ERROR (0x00000023): недостаточно аргументов"
                )
                logger.write(
                    f"[{datetime.datetime.now()}] Failed execute '{cmd[0]}' code 0x00000023\n"
                )
                logger.flush()
            ...
            if len(cmd) == 2:
                ...
                with os.scandir(modules_dir) as data_fld:
                    ...
                    for entry in data_fld:
                        if entry.is_dir() and entry.name == cmd[1]:
                            ...
                            os.system("cls")
                            try:
                                subprocess.call([f"{module}", username])
                            except Exception as e:
                                print(
                                    "MODULE_CORRUPTED_ERROR (0x00000043): модуль поврежден. Переустановите его!"
                                )
                                print(f"BUILTIN_ERROR: {e}")
                                logger.write(
                                    f"[{datetime.datetime.now()}] Failed execute '{cmd[0]}' code 0x00000043 (BUILTIN_ERROR: {e})\n"
                                )
                                logger.flush()
                            else:
                                os.system("cls")
                                logger.write(
                                    f"[{datetime.datetime.now()}] Succesful execute module '{cmd[1]}'\n"
                                )
                        else:
                            continue
                    if flag:
                        ...
                        print(
                            "MODULE_UWNKOWN_ERROR (0x00000041): неизвестный модуль. Скачайте его с помощью rolton-venv-get"
                        )
                        logger.write(
                            f"[{datetime.datetime.now()}] Failed execute '{cmd[0]}' code 0x00000041\n"
                        )
                        logger.flush()
                ...
                logger.write(
                    f"[{datetime.datetime.now()}] Failed execute '{cmd[0]}' code 0x00000042\n"
                )
                logger.flush()
            ...
            if len(cmd) == 1:
                vers_file = open(
                    os.path.join(os.getcwd(), "bin", "sys", "data", "version.txt"), "r"
                )
                ...
                if cmd[1] == "log":
                    if cmd[2] == "get":
                        ...
                        if root:
                            ...
                            logger = open(
                                os.path.join(
                                    os.getcwd(), "bin", "sys", "data", "log.txt"
                                ),
                                "w",
                            )
                            logger.write()
                            ...
                            print(
                                "ACCESS_NOT_GRANTED_ERROR (0x00000061): вы не администратор"
                            )
                            logger.write(
                                f"[{datetime.datetime.now()}] Failed execute '{cmd[0]}' code 0x00000061\n"
                            )
                            logger.flush()
                    else:
                        print(
                            f"COMMAND_ARGUMENT_ERROR (0x00000022): неизвестный аргумент функции '{cmd[2]}'"
                        )
                        logger.write(
                            f"[{datetime.datetime.now()}] Failed execute '{cmd[0]}' code 0x00000022\n"
                        )
                        ...
                        if cmd[2] == "welcome-text":
                            if cmd[3] == "style":
                                if cmd[4] == "get":
                                    ...
                                    if root:
                                        try:
                                            ...
                                            if cmd[5] in ("standart", "doom", "money"):
                                                ...
                                                print(
                                                    f"COMMAND_ARGUMENT_ERROR (0x00000022): неизвестный аргумент функции '{cmd[5]}'"
                                                )
                                                logger.write(
                                                    f"[{datetime.datetime.now()}] Failed execute '{cmd[0]}' code 0x00000022\n"
                                                )
                                    ...
                                    print(
                                        f"COMMAND_ARGUMENT_ERROR (0x00000022): неизвестный аргумент функции '{cmd[4]}'"
                                    )
                                    logger.write(
                                        f"[{datetime.datetime.now()}] Failed execute '{cmd[0]}' code 0x00000022\n"
                                    )
                                ...
                                print(
                                    f"COMMAND_ARGUMENT_ERROR (0x00000022): неизвестный аргумент функции '{cmd[3]}'"
                                )
                                logger.write(
                                    f"[{datetime.datetime.now()}] Failed execute '{cmd[0]}' code 0x00000022\n"
                                )
                                    ...
                                    if len(cmd) > 6:
                                        if cmd[6] in ("1", "0"):
                                            workdir = os.path.join(
                                                os.getcwd(), "users", cmd[4]
                                            )
                                            try:
                                                os.mkdir(workdir)
                                            except FileExistsError:
                                                print(
                                                    f"FILE_EXISTS_ERROR (0x00000011): пользователь '{cmd[4]}' уже существует"
                                                )
                                                logger.write(
                                                    f"[{datetime.datetime.now()}] Failed execute '{cmd[0]}' code 0x00000011\n"
                                                )
                                                logger.flush()
                                            code, error, _ = registr(
                                                cmd[4],
                                                cmd[5],
                                                workdir,
                                                "40",
                                            ...
                                            if code:
                                                ...
                                                logger.write(
                                                    f"[{datetime.datetime.now()}] Failed execute '{cmd[0]}' (BUILTIN_ERROR: '{error}')\n"
                                                )
                                                logger.flush()
                                        else:
                                            print(
                                                f"COMMAND_ARGUMENT_ERROR (0x00000022): неизвестный аргумент функции '{cmd[6]}'"
                                            )
                                            logger.write(
                                                f"[{datetime.datetime.now()}] Failed execute '{cmd[0]}' code 0x00000022\n"
                                            )
            ...
            if len(cmd) == 1:
                ...
                try:
                    script_user = open(
                        os.path.join(os.getcwd(), "scripts", f"{cmd[1]}")
                    )
                except FileNotFoundError:
                    print("FILE_NOT_FOUND_ERROR (0x00000051): файл не найден!")
                    logger.write(
                        f"[{datetime.datetime.now()}] Failed execute '{cmd[0]}' code 0x00000051\n"
                    )
                    logger.flush()
                except Exception as e:
                    logger.write(
                    ...
                    for cmd in commands:
                        runner(cmd, workdir, userdir, username)
                    print()
                    script_user.close()
                    logger.write(
                        f"[{datetime.datetime.now()}] Succesful execute '{cmd[0]}'\n"
            ...
            if len(cmd) == 1:
                fetch_file = open(
                    os.path.join(os.getcwd(), "bin", "sys", "data", "fetch.txt"), "r"
                )
            ...
            print(
                f"COMMAND_UWNKOWN_ERROR (0x00000021): неизвестная команда '{cmd[0]}'. Для запуска модуля используйте 'module-run', для запуска утилиты 'util-run'"
            )
            logger.write(
                f"[{datetime.datetime.now()}] Failed execute '{cmd[0]}' code 0x00000021\n"
            )
            logger.flush()
    else:
        return
